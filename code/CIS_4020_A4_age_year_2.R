...
# Get counts of non-NA entries for each column
non_na_counts <- colSums(!is.na(reduced_model))

# Print or use the counts
print(non_na_counts)


non_na_counts <- rowSums(!is.na(reduced_model))

# Identify rows with at least 5 non-NA values
#reduced_model <- reduced_model[non_na_counts >= 5, ]

# Print or use the rows
#print(reduced_model)
...
for (col in names(reduced_model)) {
  ...
  # antibiotic columns
  if (col != "age_year"){
    #reduced_model[[col]] <- recode(reduced_model[[col]], "I" = 3, "R" = 1, "S" = 2, .default = NA_real_)
    ...
    #for (i in 1:(length(reduced_model[[col]]))){
      # Create a mapping of string values to numerical values
      # Use recode to map values
      #reduced_model[[col]] <- recode(reduced_model[[col]], "I" = 3, "R" = 1, "S" = 2, .default = 0)

      # get rid of the string representation of less than a year
      #if (reduced_model[[col]][i] == "R"){
      #  
      #  reduced_model[[col]][i] <- as.integer(1)
      #  
      #} else if (reduced_model[[col]][i] == "S") {
      #  reduced_model[[col]][i] <- as.integer(2)
      #} else if (reduced_model[[col]][i] == "I") {
      #  reduced_model[[col]][i] <- as.integer(3)
      #}
      #else {
      #  reduced_model[[col]][i] = as.integer(0)
      #}
    # convert Antibioitc to factor to work with linear regression
    #reduced_model[[col]] <- factor(reduced_model[[col]], levels = c("I", "R", "S", "N/I", "NA"))
    #}
...
actual_values <- as.numeric(reduced_model$age_year)
